...
UK36_Tags <- which(Known_Tags$Strain_ID == "CP031289.1")
UK36_Tags <- Known_Tags[UK36_Tags,]
...
Library <- table(UK36_Tags$Upstream_Tag)
...
args[3]='UK36_Upstream_Unique_Tags.csv'
...
write.csv(investigate, "UK36_Extra_IS_Not_Unique.csv")
write.table(investigate, paste(args[2], "_Known_New_Upstream_Tags.csv", sep=""), row.names = F,quote = F,sep = ",")
